...
// 2. Even if the derived is not declared virtual and has base function virtual,
// derived is virtual
...
//     --> No need to make destructor virtual if new objects are not getting
//     created using new
...
class Base {
public:
  virtual void print() { std::cout << "In Base::print" << std::endl; }
...
public:
  void print(int x) { std::cout << "In Derived::print(int)" << std::endl; }
...
int main() {
  Base *p = new Derived();
  // p->print(0); // compilation error because print in derived has different
  // signature
