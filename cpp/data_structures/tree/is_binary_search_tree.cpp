...
struct Tree  
{
    ...
    void set_left(Tree* left_tree)
    {
        _left = left_tree;
    }

    void set_right(Tree* right_tree)
    {
        _right = right_tree;
    }
   ...
   bool validate_left_subtree(Tree *p, T& val) 
   {
       if (!p) 
            return true;
       return (p->_data < val &&
               validate_left_subtree(p->_left, val) &&
   ...
   bool validate_right_subtree(Tree *p, T& val) {
       if (!p) return true;
       return (p->_data > val &&
               validate_right_subtree(p->_left, val) &&
   ...
   bool isBST(Tree *p) {
       ...
       return validate_left_subtree(p->_left, p->_data) &&
           validate_right_subtree(p->_right, p->_data) &&
           isBST(p->_left) &&
           isBST(p->_right);
   } 
