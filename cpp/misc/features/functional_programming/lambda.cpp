...
int main()
{
  ...
  int x=10;
  auto lambda_with_capture_by_val_only_x = 
    [x] () { std::cout << "Lambda with value " << x << std::endl;} ;
  lambda_with_capture_by_val_only_x();

  int y =20;
  auto lambda_capture_all_locals_by_value = 
    [=] () { std::cout << "Lambda with all locals captured. x= " <<  x << ", y = " << y << std::endl;};
  lambda_capture_all_locals_by_value();
  
  auto lambda_capture_all_locals_by_value_and_ref = 
    [x,&y] () { y++; std::cout << "Lambda with all locals captured. x= " <<  x << ", y = " << y << std::endl;};
  lambda_capture_all_locals_by_value_and_ref();
  
  auto lambda_capture_all_locals_by_ref = 
    [&] () { x++; y++; std::cout << "Lambda with all locals captured. x= " <<  x << ", y = " << y << std::endl;};
