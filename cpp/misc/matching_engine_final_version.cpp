...
struct Order {
  ...
  Order(/*const OrderSide::ORDER_SIDE& s, */ const OrderType::ORDER_TYPE t,
        const QUANTITY &q, const ORDER_ID &id)
      : order_type(t),
        // order_side(s),
        quantity(q), order_id(id) {
    // Do nothing
  }
};
  ...
  void process(const std::string &input_line) {
    ...
    if ("BUY" == tokens[0]) {
      matchBuyOrder(/*OrderSide::toOrderSide(tokens[0]),*/
                    OrderType::toOrderType(tokens[1]), std::stoi(tokens[2]),
    ...
    if ("MODIFY" == tokens[0]) {
      modifyOrder(tokens[1], OrderSide::toOrderSide(tokens[2]),
...
private:
  void matchSellOrder(/*const OrderSide::ORDER_SIDE order_side,*/
    ...
    for (auto buy_order_iterator = buy_orders.begin();
      ...
      if (sell_price <= buy_price) {
        ...
        if (buy_order->quantity <= total_sell_quantity) {
          std::cout << "TRADE"
                    << " " << buy_order->order_id << " " << buy_price << " "
                    << buy_order->quantity << " " << order_id << " "
                    << sell_price << " " << buy_order->quantity << std::endl;
          ...
          std::cout << "TRADE"
                    << " " << buy_order->order_id << " " << buy_price << " "
                    << total_sell_quantity << " " << order_id << " "
                    << sell_price << " " << total_sell_quantity << std::endl;
    ...
    if (total_sell_quantity != 0 && order_type != OrderType::ORDER_TYPE::IOC) {
      Order *new_sell_order =
          new Order(/*order_side,*/ order_type, total_sell_quantity, order_id);
  ...
  void matchBuyOrder(/*const OrderSide::ORDER_SIDE order_side,*/
                     const OrderType::ORDER_TYPE order_type,
                     const PRICE &buy_price, const QUANTITY &buy_quantity,
                     const ORDER_ID &order_id) {
    ...
    for (auto sell_order_iterator = sell_orders.begin();
      ...
      if (buy_price >= sell_price) {
        ...
        if (sell_order->quantity <= total_buy_quantity) {
          std::cout << "TRADE"
                    << " " << sell_order->order_id << " " << sell_price << " "
                    << sell_order->quantity << " " << order_id << " "
                    << buy_price << " " << sell_order->quantity << std::endl;
          ...
          std::cout << "TRADE"
                    << " " << sell_order->order_id << " " << sell_price << " "
                    << total_buy_quantity << " " << order_id << " " << buy_price
                    << " " << total_buy_quantity << std::endl;
    ...
    // Some or all quantity pending to match for the given price
    if (total_buy_quantity != 0 && order_type != OrderType::ORDER_TYPE::IOC) {
      Order *new_buy_order =
          new Order(/*order_side,*/ order_type, total_buy_quantity, order_id);
  ...
  void cancelOrder(const ORDER_ID &order_id_to_cancel) {
    // Check buy side
    for (auto buy_order = buy_orders.begin(); buy_order != buy_orders.end();
         buy_order++) {
    ...
    // Check sell side
    for (auto sell_order = sell_orders.begin(); sell_order != sell_orders.end();
         sell_order++) {
...

  {
    ...
    (OrderSide::ORDER_SIDE::BUY == order_side)
        ? matchBuyOrder(
              /*OrderSide::ORDER_SIDE::BUY,*/ OrderType::ORDER_TYPE::GFD,
              new_price, new_quantity, order_id_to_modify)
        : matchSellOrder(
              /*OrderSide::ORDER_SIDE::SELL,*/ OrderType::ORDER_TYPE::GFD,
              new_price, new_quantity, order_id_to_modify);
  }
