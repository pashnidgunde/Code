...
public:
	void process(const std::string& input_line)
	{
		...
		if ("BUY" == tokens[0])
		{
			matchBuyOrder(Order::toOrderType(tokens[1]),
						  std::stoi(tokens[2]),
					      std::stoi(tokens[3]),
					      tokens[4]);
		}
		...
		if ( "SELL" == tokens[0])
		{
			matchSellOrder(Order::toOrderType(tokens[1]),
						  std::stoi(tokens[2]),
						  std::stoi(tokens[3]),
						  tokens[4]);
		}
...
private:
	...
	void printTrade(const ORDER_ID & order_id1,const PRICE& price1, const QUANTITY& quantity1 ,
				    const ORDER_ID & order_id2,const PRICE& price2, const QUANTITY& quantity2 )
	{
		std::cout << "TRADE"   << " "
				  << order_id1 << " "
				  << price1    << " "
					   ...
					   const ORDER_ID & order_id)
	{
				...
				buy_order_iterator++ )
		{
			...
			if ( sell_price <= buy_price )
			{
				...
				{
					this->printTrade(buy_order->order_id,buy_price,buy_order->quantity,order_id,sell_price,buy_order->quantity);
				...
				{
					this->printTrade(buy_order->order_id,buy_price,total_sell_quantity,order_id,sell_price,total_sell_quantity);
		...
		if (total_sell_quantity != 0 &&
			order_type != Order::ORDER_TYPE::IOC)
		{
			_sell_orders.insert(std::pair<PRICE, std::unique_ptr<Order>> (sell_price , std::unique_ptr<Order>(new Order(order_type,total_sell_quantity,order_id) )));
		}
	}

	void matchBuyOrder(const Order::ORDER_TYPE order_type,
			   ...
			   const ORDER_ID & order_id)
	{
				...
				sell_order_iterator++ )
		{
			...
			if ( buy_price >= sell_price )
			{
				...
				{
					this->printTrade(sell_order->order_id,sell_price,sell_order->quantity,order_id,buy_price,sell_order->quantity);
				...
				{
					this->printTrade(sell_order->order_id,sell_price,total_buy_quantity,order_id,buy_price,total_buy_quantity);
		...
		if (total_buy_quantity != 0 &&
			order_type != Order::ORDER_TYPE::IOC)
		{
			_buy_orders.insert(std::pair<PRICE,std::unique_ptr<Order> > (buy_price, std::unique_ptr<Order> (new Order(order_type,total_buy_quantity,order_id))));
		}
	}
	...
	void cancelOrder(const ORDER_ID & order_id_to_cancel)
	{
		...
		// Check buy side
		for (auto buy_order = _buy_orders.begin(); buy_order != _buy_orders.end(); buy_order++)
		{
		...
		// Check sell side
		for (auto sell_order = _sell_orders.begin(); sell_order != _sell_orders.end(); sell_order++)
		{
	...
	void modifyOrder(const ORDER_ID & order_id_to_modify,
					 const Order::ORDER_SIDE order_side,
...

	{
		...
		(Order::ORDER_SIDE::BUY == order_side ) ?
				matchBuyOrder(Order::ORDER_TYPE::GFD,new_price,new_quantity,order_id_to_modify) :
				matchSellOrder(Order::ORDER_TYPE::GFD,new_price,new_quantity,order_id_to_modify);
	}
