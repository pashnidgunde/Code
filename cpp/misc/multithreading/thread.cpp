...
int main() {
  ...
  Functor f;
  // call with string
  std::string str("This value is not changed in thread , even if passed by "
                  "reference. You need to use std::ref(X) ");

  // The line below will not compile because value is passed , but thread
  // accepts refrence.
  ...
  // Note that you can use std::move to move the str from main thread to child
  // thread
  // This did not work on clang and g++, however seems to work on visual c++ (
  // as per youtube tutorial )
