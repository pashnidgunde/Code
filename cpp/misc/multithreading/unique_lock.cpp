...
void thread_function() {
  ...
  // You can defer the lock as below. Which means, you have the lock but its
  // unlocked.
  // std::unique_lock<std::mutex> lock(x, std::defer_lock);

  std::cout << "Hello from thread. Unique locks can not be copied.";
  ...
  // Unlike lock_guard that applies gurard to whole function, you can lock
  // specific section.
  lock1.unlock();
